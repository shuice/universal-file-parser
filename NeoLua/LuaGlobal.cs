...
namespace Neo.IronLua
{
	...
	public class LuaGlobal : LuaTable
	{
		...
		public LuaResult DoChunk(string fileName, params MyKeyValuePair<string, object>[] args)
		{
		...
		public LuaResult DoChunk(string code, string name, params MyKeyValuePair<string, object>[] args)
		{
		...
		public LuaResult DoChunk(TextReader tr, string name, params MyKeyValuePair<string, object>[] args)
		{
			// Erzeuge die Parameter
			object[] callArgs;
			MyKeyValuePair<string, Type>[] callTypes;
			if (args != null)
			{
				callArgs = new object[args.Length];
				callTypes = new MyKeyValuePair<string, Type>[args.Length];
				for (var i = 0; i < args.Length; i++)
				{
					callArgs[i] = args[i].Value;
					callTypes[i] = new MyKeyValuePair<string, Type>(args[i].Key, args[i].Value == null ? typeof(object) : args[i].Value.GetType());
				}
			}
			else
			{
				callArgs = new object[0];
				callTypes = new MyKeyValuePair<string, Type>[0];
			}
		...
		private sealed class ArrayIndexEnumerator : IEnumerable<MyKeyValuePair<int, object>>
		{
			...
			public IEnumerator<MyKeyValuePair<int, object>> GetEnumerator()
			{
				var i = 1;
				foreach (var c in array)
					yield return new MyKeyValuePair<int, object>(i++, c);
			} // func GetEnumerator
		...
		internal static MyKeyValuePair<string, object>[] CreateArguments(int offset, object[] args)
		{
			var p = new MyKeyValuePair<string, object>[(args.Length - offset + 1) / 2]; // on 3 arguments we have 1 parameter
			...
			for (var i = 0; i < p.Length; i++)
			{
				...
				p[i] = new MyKeyValuePair<string, object>(name, value);
			}
		...
		private object LuaLoad(object ld, string source, string mode, LuaTable env)
		{
			...
			try
			{
				...
				// create the chunk
				return LuaLoadReturn(Lua.CompileChunk((string)ld, source, DefaultCompileOptions, new MyKeyValuePair<string, Type>("...", typeof(object[]))), env);
			}
			catch (Exception e)
			{
		...
		private object LuaLoadFile(string filename, string mode, LuaTable env)
		{
			...
			// create the chunk
			return LuaLoadReturn(Lua.CompileChunk(filename, DefaultCompileOptions, new MyKeyValuePair<string, Type>("...", typeof(object[]))), env);
		} // func LuaLoadFile
		...
		internal readonly MyDictionary<object, object> loaded = new MyDictionary<object, object>();
		...
		public static IEnumerable<MyKeyValuePair<string, object>> LuaRawMembers(LuaTable t)
		{
			if (t == null)
		...
		private static LuaResult PairsEnum<TKey>(object s, object current)
		{
			...
			if (e.MoveNext())
			{
				var k = (MyKeyValuePair<TKey, object>)e.Current;
