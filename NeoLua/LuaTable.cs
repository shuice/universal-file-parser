...
namespace Neo.IronLua
{
	...
	public class LuaTable : IDynamicMetaObjectProvider, INotifyPropertyChanged, IMyDictionary<object, object>
	{
		...
		private sealed class MemberImplementation : IMyDictionary<string, object>
		{
			private readonly LuaTable table;
			...
			#region -- IMyDictionary<string, object> members ------------------------------
			...
			#region -- ICollection<MyKeyValuePair<string, object>> members ----------------

			public void Add(MyKeyValuePair<string, object> item)
			{
				if (item.Key == null)
					throw new ArgumentNullException(Properties.Resources.rsTableKeyNotNullable);
			...
			public bool Remove(MyKeyValuePair<string, object> item)
			{
				if (item.Key == null)
					throw new ArgumentNullException(Properties.Resources.rsTableKeyNotNullable);
			...
			public bool Contains(MyKeyValuePair<string, object> item)
				=> table.ContainsMember(item.Key);

			public void CopyTo(MyKeyValuePair<string, object>[] array, int arrayIndex)
			...
			#region -- IEnumerator<MyKeyValuePair<string, object>> members ----------------

			public IEnumerator<MyKeyValuePair<string, object>> GetEnumerator()
		...
		private readonly MyDictionary<int, CallSite> callSites = new MyDictionary<int, CallSite>(); // call site for calls
		...
		private void MembersCopyTo(MyKeyValuePair<string, object>[] array, int arrayIndex)
		{
			...
			for (var i = hiddenMemberCount; i < entries.Length; i++)
			{
				if (entries[i].key is string member)
					array[arrayIndex++] = new MyKeyValuePair<string, object>(member, entries[i].value);
			}
		} // proc MembersCopyTo

		private IEnumerator<MyKeyValuePair<string, object>> MembersGetEnumerator()
		{
			...
			for (var i = hiddenMemberCount; i < entries.Length; i++)
			{
				...
				if (entries[i].key is string member)
					yield return new MyKeyValuePair<string, object>(member, entries[i].value);
			}
		...
		void IMyDictionary<object, object>.Add(object key, object value)
		{
			if (ContainsKey(key))
				throw new ArgumentException(String.Format(Properties.Resources.rsTableAddDuplicate, key));

			SetValue(key, value, true);
		} // proc IMyDictionary<object, object>.Add
		...
		bool IMyDictionary<object, object>.Remove(object key)
		{
			if (ContainsKey(key))
			{
				...
				return false;
		} // func IMyDictionary<object, object>.Remove
		...
		} // IMyDictionary<object, object>.Keys
		...
		} // func IMyDictionary<object, object>.Values

		object IMyDictionary<object, object>.this[object key]
		{
			get { return GetValue(key, true); }
			set { SetValue(key, value, true); }
		} // prop IMyDictionary<object, object>.this
		...
		protected virtual IEnumerable<string> GetKeys()
			=> ((IMyDictionary<object, object>)this).Keys.Select(c => (string)Lua.RtConvertValue(c, typeof(string)));

		#endregion

		#region -- ICollection<MyKeyValuePair<object, object>> ------------------------------
		...
		} // proc ICollection<MyKeyValuePair<object, object>>.Add
		...
		bool ICollection<MyKeyValuePair<object, object>>.Remove(MyKeyValuePair<object, object> item)
		{
			if (ContainsKey(item.Key))
			{
				...
				return false;
		} // func ICollection<MyKeyValuePair<object, object>>.Remove

		void ICollection<MyKeyValuePair<object, object>>.Clear()
		{
			Clear();
		} // proc ICollection<MyKeyValuePair<object, object>>.Clear
		...
		void ICollection<MyKeyValuePair<object, object>>.CopyTo(MyKeyValuePair<object, object>[] array, int arrayIndex)
		{
			...
			{
				if (arrayList[i] != null)
					array[arrayIndex++] = new MyKeyValuePair<object, object>(i + 1, arrayList[i]);
			}
			...
			for (var i = hiddenMemberCount; i < classDefinition.Count; i++)
			{
				...
				if (value != null)
					array[arrayIndex++] = new MyKeyValuePair<object, object>(entries[i].key, value);
			}
			...
			for (var i = classDefinition.Count; i < entries.Length; i++)
			{
				if (entries[i].hashCode != -1)
					array[arrayIndex++] = new MyKeyValuePair<object, object>(entries[i].key, entries[i].value);
			}
		} // proc ICollection<MyKeyValuePair<object, object>>.CopyTo

		int ICollection<MyKeyValuePair<object, object>>.Count => count - hiddenMemberCount;
		bool ICollection<MyKeyValuePair<object, object>>.IsReadOnly => false;
		...
		/// <summary></summary>
		/// <returns></returns>
		public IEnumerator<MyKeyValuePair<object, object>> GetEnumerator()
		{
			...
			for (int i = 0; i < arrayList.Length; i++)
			{
				...
				if (arrayList[i] != null)
					yield return new MyKeyValuePair<object, object>(i + 1, arrayList[i]);
			}
			...
			for (int i = hiddenMemberCount; i < classDefinition.Count; i++)
			{
				...
				object value = GetClassMemberValue(i, null, true);
				if (value != null)
					yield return new MyKeyValuePair<object, object>(entries[i].key, value);
			}
			...
			for (int i = classDefinition.Count; i < entries.Length; i++)
			{
				...
				if (entries[i].hashCode != -1)
					yield return new MyKeyValuePair<object, object>(entries[i].key, entries[i].value);
			}
		} // func IEnumerator<MyKeyValuePair<object, object>>
		...
		public IMyDictionary<string, object> Members => new MemberImplementation(this);
		/// <summary>Access to all values.</summary>
		public IMyDictionary<object, object> Values => this;
		...
		public static T[] collect<T>(LuaTable t, int i, int j, T[] empty)
		{
			...
			{
				var indexList = new List<MyKeyValuePair<int, T>>(Math.Max(Math.Min(j - i + 1, t.count), 1));
				...
				if (i <= t.arrayList.Length && j >= 1)
				{
					...
					for (int k = idxStart; k <= idxEnd; k++)
						if (t.arrayList[k] != null)
							indexList.Add(new MyKeyValuePair<int, T>(k + 1, (T)Lua.RtConvertValue(t.arrayList[k], typeof(T))));
				}
				...
				for (var k = 0; k < t.entries.Length; k++)
				{
					if (t.entries[k].key is int)
					{
						...
						if (l >= i && l <= j)
							indexList.Add(new MyKeyValuePair<int, T>(l, (T)Lua.RtConvertValue(t.entries[k].value, typeof(T))));
					}
				}
