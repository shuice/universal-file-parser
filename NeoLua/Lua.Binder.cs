...
namespace Neo.IronLua
{
	...
	public partial class Lua
	{
		...
		private readonly MyDictionary<ExpressionType, CallSiteBinder> operationBinder = new MyDictionary<ExpressionType, CallSiteBinder>();
		private readonly MyDictionary<string, CallSiteBinder> getMemberBinder = new MyDictionary<string, CallSiteBinder>();
		private readonly MyDictionary<string, CallSiteBinder> setMemberBinder = new MyDictionary<string, CallSiteBinder>();
		private readonly MyDictionary<CallInfo, CallSiteBinder> getIndexBinder = new MyDictionary<CallInfo, CallSiteBinder>();
		private readonly MyDictionary<CallInfo, CallSiteBinder> setIndexBinder = new MyDictionary<CallInfo, CallSiteBinder>();
		private readonly MyDictionary<CallInfo, CallSiteBinder> invokeBinder = new MyDictionary<CallInfo, CallSiteBinder>();
		private readonly MyDictionary<MemberCallInfo, CallSiteBinder> invokeMemberBinder = new MyDictionary<MemberCallInfo, CallSiteBinder>();
		private readonly MyDictionary<Type, CallSiteBinder> convertBinder = new MyDictionary<Type, CallSiteBinder>();
		...
		private void DumpRuleCache<T>(TextWriter tw, MyDictionary<T, CallSiteBinder> binder, FieldInfo fiCache)
		{
			lock (binder)
			{
				foreach (var c in binder)
				{
					...
					// get the cache
					var cache = (MyDictionary<Type, object>)fiCache.GetValue(c.Value);
					if (cache == null)
						continue;
