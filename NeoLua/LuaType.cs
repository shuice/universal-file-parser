...
namespace Neo.IronLua
{
	...
	public sealed class LuaType : IDynamicMetaObjectProvider, IEquatable<Type>
	{
		...
		private MyDictionary<string, int> namespaceIndex = null; // Index to speed up the search in big namespaces
			...
			if (index == -1)
			{
				...
				lock (currentTypeLock)
				{
					if (namespaceIndex == null)
						namespaceIndex = new MyDictionary<string, int>();
		...
		private sealed class AssemblyCacheList : IEnumerable<Assembly>, ILuaTypeResolver
		{
			...
			private readonly MyDictionary<string, CacheItem> cache = new MyDictionary<string, CacheItem>(StringComparer.OrdinalIgnoreCase);
			...
			private void UpdateLoadedAssembly(CacheItem item, Assembly assembly)
			{
				...
				// UWP can not use function GetReferencedAssemblies
		...
		private static readonly MyDictionary<Type, int> knownTypes = new MyDictionary<Type, int>(); // index for well known types
		private static readonly MyDictionary<string, int> knownTypeStrings = new MyDictionary<string, int>(); // index for well known types
