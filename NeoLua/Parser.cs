...
namespace Neo.IronLua
{
	...
	internal static partial class Parser
	{
		...
		private class Scope
		{
			...
			private MyDictionary<string, Expression> scopeVariables = null; // local declared variables or const definitions
			...
			private void RegisterVariableOrConst(string name, Expression expr)
			{
				if (scopeVariables == null)
					scopeVariables = new MyDictionary<string, Expression>();
		...
		private class LambdaScope : Scope
		{
			...
			private MyDictionary<string, LabelTarget> labels = null;
			private MyDictionary<string, ParameterExpression> parameters = new MyDictionary<string, ParameterExpression>();
			...
			public override LabelTarget LookupLabel(Type type, string name)
			{
				if (name == csReturnLabel)
					return returnLabel;
				if (labels == null)
					labels = new MyDictionary<string, LabelTarget>();
				if (type == null)
					type = typeof(void);
		...
		public static LambdaExpression ParseChunk(Lua runtime, LuaCompileOptions options, bool hasEnvironment, ILuaLexer code, Type typeDelegate, Type returnType, IEnumerable<MyKeyValuePair<string, Type>> args)
		{
