...
namespace AnyPrefix.Microsoft.Scripting.Hosting {
    ...
    public sealed class ScriptScope : MarshalByRefObject, IDynamicMetaObjectProvider {
        ...
        /// </summary>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1024:UsePropertiesWhereAppropriate")]
        public IEnumerable<MyKeyValuePair<string, dynamic>> GetItems() {
            // Remoting: we eagerly enumerate all variables to avoid cross domain calls for each item.
            var result = new List<MyKeyValuePair<string, object>>();
            
            foreach (string name in GetVariableNames()) {
                result.Add(new MyKeyValuePair<string, object>(name, (object)Engine.Operations.GetMember((object)Scope.Storage, name)));
            }
