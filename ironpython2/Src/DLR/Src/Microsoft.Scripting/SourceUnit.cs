...
namespace AnyPrefix.Microsoft.Scripting {
    ...
    public sealed class SourceUnit {
        ...
        private MyKeyValuePair<int, int>[] _lineMap;
        ...
        private static int BinarySearch<T>(MyKeyValuePair<int, T>[] array, int line) {
            int match = Array.BinarySearch(array, new MyKeyValuePair<int, T>(line, default(T)), new KeyComparer<T>());
            if (match < 0) {
        ...
        private class KeyComparer<T1> : IComparer<MyKeyValuePair<int, T1>> {
            public int Compare(MyKeyValuePair<int, T1> x, MyKeyValuePair<int, T1> y) {
                return x.Key - y.Key;
            }
        }
        ...
        public void SetLineMapping(MyKeyValuePair<int, int>[] lineMap) {
