...
namespace AnyPrefix.Microsoft.Scripting.Runtime {
    ...
    internal sealed class LanguageConfiguration {
        private readonly IMyDictionary<string, object> _options;
        ...
        public LanguageConfiguration(AssemblyQualifiedTypeName providerName, string displayName, IMyDictionary<string, object> options) {
    ...
    public sealed class DlrConfiguration {
        ...
        private readonly MyDictionary<string, LanguageConfiguration> _languageNames;
        private readonly MyDictionary<string, LanguageConfiguration> _languageExtensions;
        private readonly MyDictionary<AssemblyQualifiedTypeName, LanguageConfiguration> _languageConfigurations;
        private readonly MyDictionary<Type, LanguageConfiguration> _loadedProviderTypes;

        public DlrConfiguration(bool debugMode, bool privateBinding, IMyDictionary<string, object> options) {
            ContractUtils.RequiresNotNull(options, nameof(options));
            DebugMode = debugMode;
            PrivateBinding = privateBinding;
            Options = options;            
            _languageNames = new MyDictionary<string, LanguageConfiguration>(LanguageNameComparer);
            _languageExtensions = new MyDictionary<string, LanguageConfiguration>(FileExtensionComparer);
            _languageConfigurations = new MyDictionary<AssemblyQualifiedTypeName, LanguageConfiguration>();
            _loadedProviderTypes = new MyDictionary<Type, LanguageConfiguration>();
        }
        ...
        internal IMyDictionary<string, object> Options { get; }

        internal IMyDictionary<AssemblyQualifiedTypeName, LanguageConfiguration> Languages => _languageConfigurations;

        public void AddLanguage(string languageTypeName, string displayName, IList<string> names, IList<string> fileExtensions,
            IMyDictionary<string, object> options) {
        ...
        internal void AddLanguage(string languageTypeName, string displayName, IList<string> names, IList<string> fileExtensions, 
            IMyDictionary<string, object> options, string paramName) {
            ...
            var mergedOptions = new MyDictionary<string, object>(Options);
        ...
        private LanguageContext LoadLanguageContext(ScriptDomainManager manager, LanguageConfiguration config) {
            ...
            if (!alreadyLoaded) {
                ...
                lock (_loadedProviderTypes) {
                    Type type = language.GetReleaseType();
