...
namespace AnyPrefix.Microsoft.Scripting {
    ...
    public sealed class ScopeStorage : IDynamicMetaObjectProvider {
        private readonly MyDictionary<string, ScopeVariableIgnoreCase> _storage = new MyDictionary<string, ScopeVariableIgnoreCase>(StringComparer.OrdinalIgnoreCase);
        ...
        /// The list contains all available casings.
        /// </summary>
        public IList<MyKeyValuePair<string, dynamic>> GetItems() {
            List<MyKeyValuePair<string, dynamic>> res = new List<MyKeyValuePair<string, dynamic>>();
            lock (_storage) {
                foreach (var storage in _storage.Values) {
                ...
                return new DynamicMetaObject(
                    Expression.Block(
                        ...
                        Expression.Condition(
                            Expression.Call(
                                Variable(variable),
                                variable.GetReleaseType().GetMethod(nameof(IScopeVariable.TryGetValue)),
                                tmp
                            ),
            ...
            public override DynamicMetaObject BindSetMember(SetMemberBinder binder, DynamicMetaObject value) {
                ...
                return new DynamicMetaObject(
                    Expression.Block(
                        Expression.Call(
                            Variable(variable),
                            variable.GetReleaseType().GetMethod(nameof(IScopeVariable.SetValue)),
                            objExpression
                        ),
                        objExpression
                    ),
            ...
            public override DynamicMetaObject BindDeleteMember(DeleteMemberBinder binder) {
                ...
                return new DynamicMetaObject(
                    Expression.Condition(
                        Expression.Call(
                            Variable(variable),
                            variable.GetReleaseType().GetMethod(nameof(IScopeVariable.DeleteValue))
                        ),
    ...
    public sealed class ScopeVariableIgnoreCase : IScopeVariable, IWeakReferencable {
        ...
        private WeakReference _weakref;
        private MyDictionary<string, ScopeVariable> _overflow;
        ...
        internal void AddItems(List<MyKeyValuePair<string, object>> list) {
            if (_firstVariable.TryGetValue(out object value)) {
                list.Add(new MyKeyValuePair<string, object>(_firstCasing, value));
            }
            ...
            lock (_overflow) {
                foreach (var element in _overflow) {
                    if (element.Value.TryGetValue(out value)) {
                        list.Add(new MyKeyValuePair<string, object>(element.Key, value));
                    }
                }
            }
        }
        ...
        private ScopeVariable GetStorageSlow(string name) {
            if (_overflow == null) {
                Interlocked.CompareExchange(ref _overflow, new MyDictionary<string, ScopeVariable>(), null);
            }
