...
namespace AnyPrefix.Microsoft.Scripting {
    ...
    public static class PerfTrack {
        ...
        private static readonly MyDictionary<Categories, MyDictionary<string, int>> _events = MakeEventsDictionary();
        private static readonly MyDictionary<Categories, int> summaryStats = new MyDictionary<Categories, int>();

        private static MyDictionary<Categories, MyDictionary<string, int>> MakeEventsDictionary() {
            MyDictionary<Categories, MyDictionary<string, int>> result = new MyDictionary<Categories, MyDictionary<string, int>>();
            
            // We do not use Enum.GetValues here since it is not available in SILVERLIGHT 
            for (int i = 0; i <= (int)Categories.Count; i++) {
                result[(Categories)i] = new MyDictionary<string, int>();
            }
        ...
        public static void DumpHistogram<TKey>(IMyDictionary<TKey, int> histogram) {
        ...
        public static void DumpHistogram<TKey>(IMyDictionary<TKey, int> histogram, TextWriter output) {
        ...
        public static void AddHistograms<TKey>(IMyDictionary<TKey, int> result, IMyDictionary<TKey, int> addend) {
            foreach (var entry in addend) {
                result[entry.Key] = entry.Value + (result.TryGetValue(entry.Key, out int value) ? value : 0);
            }
        }

        public static void IncrementEntry<TKey>(IMyDictionary<TKey, int> histogram, TKey key) {
        ...
        public static void DumpStats(TextWriter output) {
            ...
            foreach (MyKeyValuePair<Categories, MyDictionary<string, int>> kvpCategories in _events) {
            ...
            output.WriteLine();
            double knownTimes = 0;
            foreach (MyKeyValuePair<Categories, int> kvp in summaryStats) {
                switch (kvp.Key) {
        ...
        public static void NoteEvent(Categories category, object key) {
            ...
            MyDictionary<string, int> categoryEvents = _events[category];
            totalEvents++;
            lock (categoryEvents) {
                string name = key.ToString();
                if (key is Exception ex) name = ex.GetReleaseType().ToString();
