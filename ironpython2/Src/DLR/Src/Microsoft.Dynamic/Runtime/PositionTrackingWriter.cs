...
namespace AnyPrefix.Microsoft.Scripting.Runtime {
    ...
    public class PositionTrackingWriter : StringWriter {
        List<MyKeyValuePair<int, int>> _lineMap = new List<MyKeyValuePair<int, int>>();
        List<MyKeyValuePair<int, string>> _fileMap = new List<MyKeyValuePair<int, string>>();
        ...
        public void MapLocation(CodeLinePragma linePragma) {
            _lineMap.Add(new MyKeyValuePair<int, int>(_line, linePragma.LineNumber));
            _fileMap.Add(new MyKeyValuePair<int, string>(_line, linePragma.FileName));
        }

        public MyKeyValuePair<int, int>[] GetLineMap() {
            return _lineMap.ToArray();
        }

        public MyKeyValuePair<int, string>[] GetFileMap() {
