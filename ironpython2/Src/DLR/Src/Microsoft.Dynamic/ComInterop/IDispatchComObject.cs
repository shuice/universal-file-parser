...
namespace AnyPrefix.Microsoft.Scripting.ComInterop {
    ...
    internal sealed class IDispatchComObject : ComObject, IDynamicMetaObjectProvider {
        ...
        private static readonly MyDictionary<Guid, ComTypeDesc> _CacheComTypeDesc = new MyDictionary<Guid, ComTypeDesc>();
        ...
        internal override IList<MyKeyValuePair<string, object>> GetMembers(IEnumerable<string> names) {
            if (names == null) {
                names = GetMemberNames(true);
            }

            Type comType = RuntimeCallableWrapper.GetReleaseType();

            var members = new List<MyKeyValuePair<string, object>>();
            foreach (string name in names) {
                ...
                if (ComTypeDesc.TryGetFunc(name, out ComMethodDesc method) && method.IsDataMember) {
                    try {
                        ...
                        members.Add(new MyKeyValuePair<string, object>(method.Name, value));

                        //evaluation failed for some reason. pass exception out 
                    } catch (Exception ex) {
                        members.Add(new MyKeyValuePair<string, object>(method.Name, ex));
                    }
                }
            }
        ...
        private void EnsureScanDefinedEvents() {
            ...
            MyDictionary<string, ComEventDesc> events = null;
            ...
            if (cpc == null) {
                ...
                events = new MyDictionary<string, ComEventDesc>();
        ...
        private static void ScanSourceInterface(ComTypes.ITypeInfo sourceTypeInfo, ref MyDictionary<string, ComEventDesc> events) {
