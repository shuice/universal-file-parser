...
namespace AnyPrefix.Microsoft.Scripting.Hosting.Shell.Remote {
    /// <summary>
        ...
        internal void UnhandledExceptionWorker(Exception e) {
            try {
                ...
                if (!(exceptionDuringHandling is RemotingException)) {
                    Console.WriteLine(
                        $"({exceptionDuringHandling.GetReleaseType()} thrown while trying to display unhandled exception)", Style.Error);
                }
