...
namespace AnyPrefix.Microsoft.Scripting.Interpreter {
    ...
    public struct InstructionArray {
        ...
        internal readonly List<MyKeyValuePair<int, object>> DebugCookies;

        internal InstructionArray(int maxStackDepth, int maxContinuationDepth, Instruction[] instructions, 
            object[] objects, RuntimeLabel[] labels, List<MyKeyValuePair<int, object>> debugCookies) {
    ...
    public sealed class InstructionList {
        ...
        // list of (instruction index, cookie) sorted by instruction index:
        private List<MyKeyValuePair<int, object>> _debugCookies = null;
        ...
        internal sealed class DebugView {
            ...
            internal static InstructionView[] GetInstructionViews(IList<Instruction> instructions, IList<object> objects,
                Func<int, int> labelIndexer, IList<MyKeyValuePair<int, object>> debugCookies) {

                var result = new List<InstructionView>();
                int stackDepth = 0;
                int continuationsDepth = 0;

                using (var cookieEnumerator = (debugCookies ?? EmptyArray<MyKeyValuePair<int, object>>.Instance).GetEnumerator()) {
        ...
        public void SetDebugCookie(object cookie) {
            ...
            if (_debugCookies == null) {
                _debugCookies = new List<MyKeyValuePair<int, object>>();
            }

            Debug.Assert(Count > 0);
            _debugCookies.Add(new MyKeyValuePair<int, object>(Count - 1, cookie));
#endif
        }
...
#if STATS
        private static MyDictionary<string, int> _executedInstructions = new MyDictionary<string, int>();
        private static MyDictionary<string, MyDictionary<object, bool>> _instances = new MyDictionary<string, MyDictionary<object, bool>>();
        ...
        static InstructionList() {
            AppDomain.CurrentDomain.ProcessExit += new EventHandler((_, __) => {
                ...
                var referenced = new MyDictionary<string, int>();
                int total = 0;
        ...
        private static readonly MyDictionary<FieldInfo, Instruction> _loadFields = new MyDictionary<FieldInfo, Instruction>();
        ...
        private static MyDictionary<Type, Func<CallSiteBinder, Instruction>> _factories =
            new MyDictionary<Type, Func<CallSiteBinder, Instruction>>();
        ...
        public void EmitSwitch(MyDictionary<int, int> cases) {
