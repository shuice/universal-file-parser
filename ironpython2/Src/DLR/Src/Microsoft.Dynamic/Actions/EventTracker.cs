...
namespace AnyPrefix.Microsoft.Scripting.Actions {
    public class EventTracker : MemberTracker {
        ...
        public void AddHandler(object target, object handler, DynamicDelegateCreator delegateCreator) {
            ...
            if (_eventInfo.EventHandlerType.IsAssignableFrom(handler.GetReleaseType())) {
            ...
            if (target != null) {
                add = CompilerHelpers.TryGetCallableMethod(target.GetReleaseType(), add);
            }
        ...
        private sealed class ComHandlerList : HandlerList {
            ...
            /// </summary>
            private readonly CopyOnWriteList<MyKeyValuePair<object, object>> _handlers = new CopyOnWriteList<MyKeyValuePair<object, object>>();

            public override void AddHandler(object callableObject, Delegate handler) {
                Assert.NotNull(handler);
                _handlers.Add(new MyKeyValuePair<object, object>(callableObject, handler));
            }

            public override Delegate RemoveHandler(object callableObject, IEqualityComparer<object> comparer) {
                List<MyKeyValuePair<object, object>> copyOfHandlers = _handlers.GetCopyForRead();
        ...
        private sealed class NormalHandlerList : HandlerList {
            ...
            /// </summary>
            private readonly CopyOnWriteList<MyKeyValuePair<WeakReference, WeakReference>> _handlers = new CopyOnWriteList<MyKeyValuePair<WeakReference, WeakReference>>();

            public override void AddHandler(object callableObject, Delegate handler) {
                Assert.NotNull(handler);
                _handlers.Add(new MyKeyValuePair<WeakReference, WeakReference>(new WeakReference(callableObject), new WeakReference(handler)));
            }

            public override Delegate RemoveHandler(object callableObject, IEqualityComparer<object> comparer) {
                List<MyKeyValuePair<WeakReference, WeakReference>> copyOfHandlers = _handlers.GetCopyForRead();
                for (int i = copyOfHandlers.Count - 1; i >= 0; i--) {
