...
namespace AnyPrefix.Microsoft.Scripting.Ast {
    internal sealed class LambdaParameterRewriter : ExpressionVisitor {
        private readonly MyDictionary<ParameterExpression, ParameterExpression> _map;

        internal LambdaParameterRewriter(MyDictionary<ParameterExpression, ParameterExpression> map) {
            _map = map;
        }
