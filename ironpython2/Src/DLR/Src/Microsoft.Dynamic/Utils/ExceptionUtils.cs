...
namespace AnyPrefix.Microsoft.Scripting.Utils {
    public static class ExceptionUtils {
...
#else
        private static ConditionalWeakTable<Exception, List<MyKeyValuePair<object, object>>> _exceptionData;

        public static void SetData(this Exception e, object key, object value) {
            if (_exceptionData == null) {
                Interlocked.CompareExchange(ref _exceptionData, new ConditionalWeakTable<Exception, List<MyKeyValuePair<object, object>>>(), null);
            }
            ...
            lock (_exceptionData) {
                ...
                if (index >= 0) {
                    data[index] = new MyKeyValuePair<object, object>(key, value);
                } else {
                    data.Add(new MyKeyValuePair<object, object>(key, value));
                }
            }
        }
        ...
        public static object GetData(this Exception e, object key) {
            ...
            lock (_exceptionData) {
                List<MyKeyValuePair<object, object>> data;
