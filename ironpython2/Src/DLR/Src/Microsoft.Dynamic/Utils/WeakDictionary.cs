...
namespace AnyPrefix.Microsoft.Scripting.Utils {
    ...
    /// </summary>
    public class WeakDictionary<TKey, TValue> : IMyDictionary<TKey, TValue> {
        ...
        private IMyDictionary<object, TValue> dict = new MyDictionary<object, TValue>(comparer);
        ...
        private void Cleanup() {
            ...
            if (emptyCount > liveCount / 4) {
                MyDictionary<object, TValue> newtable = new MyDictionary<object, TValue>(liveCount + liveCount / 4, comparer);
    ...
    public sealed class HybridMapping<T> {
        private MyDictionary<int, object> _dict = new MyDictionary<int, object>();
        ...
        public T GetObjectFromId(int id) {
            if (_dict.TryGetValue(id, out object ret)) {
                ...
                throw new InvalidOperationException("Unexpected dictionary content: type " + ret.GetReleaseType());
            }
        ...
        public int GetIdFromObject(T value) {
            lock (_synchObject) {
                foreach (MyKeyValuePair<int, object> kv in _dict) {
                    switch (kv.Value) {
