...
namespace AnyPrefix.Microsoft.Scripting.Metadata {
    public static class MetadataServices {
        ...
        private static MyDictionary<Assembly, MetadataTables[]> _metadataCache;

        private static MetadataTables[] GetAsseblyMetadata(Assembly assembly) {
            if (_metadataCache == null) {
                _metadataCache = new MyDictionary<Assembly, MetadataTables[]>();
            }
        ...
        public static List<MyKeyValuePair<Module, int>> GetVisibleExtensionMethods(Assembly assembly) {
            if (assembly == null) {
            ...
            var result = new List<MyKeyValuePair<Module, int>>();
            if (!eaCtor.IsNull) {
                    ...
                    if (ca.Constructor.Equals(eaCtor) && ca.Parent.IsMethodDef) {
                        ...
                        if ((mattrs & MethodAttributes.MemberAccessMask) == MethodAttributes.Public && (mattrs & MethodAttributes.Static) != 0) {
                            ...
                            if (((tattrs & TypeAttributes.VisibilityMask) == TypeAttributes.Public ||
                                ...
                                result.Add(new MyKeyValuePair<Module, int>(manifest.Module, mdef.Record.Token.Value));
                            }
                        }
                    }
                }
