    ...
    public static class _ast
    {
        ...
        internal static PythonAst ConvertToPythonAst(CodeContext codeContext, AST source, string filename) {
            ...
            if (source is Expression) {
                ...
                throw PythonOps.TypeError("unsupported type of AST: {0}",(source.GetReleaseType()));
        ...
        public abstract class AST
        {
                ...
                foreach (object name in names) {
                    if (name is string) {
                        try {
                            ...
                            this.GetReleaseType().GetProperty(key).SetValue(this, source[key], null);
            ...
            internal void storeProperties(IEnumerable<object> names, IDictionary target) {
                foreach (object name in names) {
                    if (name is string) {
                        ...
                        object val;
                        try {
                            val = this.GetReleaseType().GetProperty(key).GetValue(this, null);
                            target.Add(key, val);
            ...
            internal static stmt Convert(Statement stmt) {
                ...
                if (stmt is FunctionDefinition)
                    ...
                    throw new ArgumentTypeException("Unexpected statement type: " + stmt.GetReleaseType());
            ...
            internal static expr Convert(AstExpression expr, expr_context ctx) {
                ...
                if (expr is ConstantExpression)
                    ...
                    throw new ArgumentTypeException("Unexpected expression type: " + expr.GetReleaseType());
            ...
            internal static expr Convert(ConstantExpression expr) {
                ...
                if (expr.Value is int || expr.Value is double || expr.Value is Int64 || expr.Value is BigInteger || expr.Value is Complex)
                    ...
                    throw new ArgumentTypeException("Unexpected constant type: " + expr.Value.GetReleaseType());
            ...
            internal static expr Convert(BinaryExpression expr) {
                ...
                throw new ArgumentTypeException("Unexpected operator type: " + op.GetReleaseType());
            }
            ...
            internal static AST Convert(Node node) {
                ...
                if (node is TryStatementHandler)
                    ...
                    throw new ArgumentTypeException("Unexpected node type: " + node.GetReleaseType());
        ...
        public class Lambda : expr
        {
            ...
            internal Lambda(LambdaExpression lambda)
                ...
                if (statement is Return)
                    ...
                    throw PythonOps.TypeError("Unexpected statement type: {0}, expected Return or Expr", statement.GetReleaseType());
            }
        ...
        public class Subscript : expr
        {
            ...
            internal override AstExpression Revert() {
                ...
                if (slice is Index)
                    ...
                    Debug.Assert(false, "Unexpected type when converting Subscript: " + slice.GetReleaseType());
                }
