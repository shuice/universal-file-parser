    ...
    class RuntimeScriptCode : RunnableScriptCode {
        ...
        private Func<FunctionCode, object>/*!*/ Compile() {
            ...
            if (pc.ShouldInterpret(pco, SourceUnit)) {
                return ((AnyPrefix.Microsoft.Scripting.Ast.LightExpression<Func<FunctionCode, object>>)Ast.GetLambda()).Compile(pc.Options.CompilationThreshold);
            } else {
                return ((AnyPrefix.Microsoft.Scripting.Ast.LightExpression<Func<FunctionCode, object>>)Ast.GetLambda()).ReduceToLambda().Compile(pc.EmitDebugSymbols(SourceUnit));
            }
        }
    }
}
