...
namespace IronPython.Runtime {
    class PythonDocumentationProvider : DocumentationProvider {
        ...
        public override ICollection<MemberDoc> GetMembers(object value) {
            ...
            if (value is NamespaceTracker ns) {
                foreach (var v in ns) {
                    AddMember(
                        res,
                        new MyKeyValuePair<object, object>(
                            v.Key,
        ...
        private static void AddMember(List<MemberDoc> res, MyKeyValuePair<object, object> member, bool fromClass) {
        ...
        private static MemberDoc MakeMemberDoc(string name, object value, bool fromClass) {
            ...
            } else if (value != null && value.GetReleaseType().IsEnum) {
        ...
        public override ICollection<OverloadDoc> GetOverloads(object value) {
            ...
            if (value is Delegate dlg) {
                return new[] { DocBuilder.GetOverloadDoc(dlg.GetReleaseType().GetMethod("Invoke"), dlg.GetReleaseType().Name, 0, false) };
            }
