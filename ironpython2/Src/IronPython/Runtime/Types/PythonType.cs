...
type(name, bases, dict) -> creates a new type instance with the given name, base classes, and members from the dictionary")]
    public partial class PythonType : IPythonMembersList, IDynamicMetaObjectProvider, IWeakReferenceable, IWeakReferenceableByProxy, ICodeFormattable, IFastGettable, IFastSettable, IFastInvokable {
        ...
        private MyDictionary<string, PythonTypeSlot> _dict;   // type-level slots & attributes
        ...
        internal MyDictionary<CachedGetKey, FastGetBase> _cachedGets; // cached gets on user defined type instances
        internal MyDictionary<CachedGetKey, FastGetBase> _cachedTryGets; // cached try gets on used defined type instances
        internal MyDictionary<SetMemberKey, FastSetBase> _cachedSets; // cached sets on user defined instances
        internal MyDictionary<string, TypeGetBase> _cachedTypeGets; // cached gets on types (system and user types)
        internal MyDictionary<string, TypeGetBase> _cachedTypeTryGets; // cached gets on types (system and user types)
        ...
        private MyDictionary<CallSignature, LateBoundInitBinder> _lateBoundInitBinders;
        ...
        private MyDictionary<string, List<MethodInfo>> _extensionMethods;         // extension methods defined on the type
        ...
        /// </summary>
        private static MyDictionary<Type, MyDictionary<Type, Delegate>> _fastBindCtors = new MyDictionary<Type, MyDictionary<Type, Delegate>>();
        ...
        /// </summary>
        private static MyDictionary<Type, BuiltinFunction> _userTypeCtors = new MyDictionary<Type, BuiltinFunction>();
        ...
        public object __call__(CodeContext context, [ParamDictionary]IMyDictionary<string, object> kwArgs, params object[] args) {
        ...
        internal static PythonType SetPythonType(Type type, PythonType pyType) {
            lock (_pythonTypes) {
                ...
                Debug.Assert(pyType.GetReleaseType() != typeof(PythonType));
        ...
        internal LateBoundInitBinder GetLateBoundInitBinder(CallSignature signature) {
            ...
            if (_lateBoundInitBinders == null) {
                Interlocked.CompareExchange(ref _lateBoundInitBinders, new MyDictionary<CallSignature, LateBoundInitBinder>(), null);
            }
        ...
        internal MyDictionary<string, List<MethodInfo>> ExtensionMethods {
            get {
                if (_extensionMethods == null) {
                    MyDictionary<string, List<MethodInfo>> extMethods = new MyDictionary<string, List<MethodInfo>>();
        ...
        internal List GetMemberNames(CodeContext context, object self) {
            ...
            MyDictionary<string, string> keys = new MyDictionary<string, string>();
            res = new List();
        ...
        /// </summary>
        private static void AddUserTypeMembers(CodeContext context, MyDictionary<string, string> keys, PythonType dt, List res) {
            if (dt.OldClass != null) {
                foreach (MyKeyValuePair<object, object> kvp in dt.OldClass._dict) {
                    AddOneMember(keys, res, kvp.Key);
                }
            } else {
                foreach (MyKeyValuePair<string, PythonTypeSlot> kvp in dt._dict) {
        ...
        private static void AddOneMember(MyDictionary<string, string> keys, List res, object name) {
        ...
        /// </summary>
        private static List AddInstanceMembers(object self, MyDictionary<string, string> keys, List res) {
        ...
        private void AddSystemInterfaces(List<PythonType> mro) {
            ...
            Type[] interfaces = _underlyingSystemType.GetInterfaces();
            MyDictionary<string, Type> methodMap = new MyDictionary<string, Type>();
...
//            }
        ...
        private void EnsureDict() {
            if (_dict == null) {
                Interlocked.CompareExchange<MyDictionary<string, PythonTypeSlot>>(
                    ref _dict,
                    new MyDictionary<string, PythonTypeSlot>(StringComparer.Ordinal),
                    null);
            }
        }
