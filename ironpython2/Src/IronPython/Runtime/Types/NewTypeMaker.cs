...
namespace IronPython.Runtime.Types {
    ...
    sealed class NewTypeMaker {
        ...
        [MultiRuntimeAware]
        private static readonly MyDictionary<Type, MyDictionary<string, List<MethodInfo>>> _overriddenMethods = new MyDictionary<Type, MyDictionary<string, List<MethodInfo>>>();
        [MultiRuntimeAware]
        private static readonly MyDictionary<Type, MyDictionary<string, List<ExtensionPropertyTracker>>> _overriddenProperties = new MyDictionary<Type, MyDictionary<string, List<ExtensionPropertyTracker>>>();
        ...
        public static void SaveNewTypes(string assemblyName, IList<PythonTuple> types) {
            ...
            foreach (var v in types) {
                ...
                ilg.Emit(OpCodes.Newobj, typeof(MyDictionary<string, string[]>).GetConstructor(new Type[0]));
                foreach (var specialName in typeInfos.Value) {
                    ...
                    // assign to dict
                    ilg.Emit(OpCodes.Call, typeof(MyDictionary<string, string[]>).GetMethod("set_Item"));                    
                }
        ...
        private Type CreateNewType() {
            ...
            MyDictionary<string, string[]> specialNames = ImplementType();
        ...
        private MyDictionary<string, string[]> ImplementType() {
            DefineInterfaces();
            ...
            MyDictionary<string, string[]> specialNames = new MyDictionary<string, string[]>();

            OverrideMethods(_baseType, specialNames);

            ImplementProtectedFieldAccessors(specialNames);

            MyDictionary<Type, bool> doneTypes = new MyDictionary<Type, bool>();
            foreach (Type interfaceType in _interfaceTypes) {
        ...
        private void DoInterfaceType(Type interfaceType, MyDictionary<Type, bool> doneTypes, MyDictionary<string, string[]> specialNames) {
            ...
            for (int i = 0; i < overrideParams.Length; i++) {
                ...
                if (origIndex >= 0) {
                    ...
                    if ((pi.Attributes & ParameterAttributes.HasDefault) != 0) {
                        if (pi.DefaultValue == null || pi.ParameterType.IsAssignableFrom(pi.DefaultValue.GetReleaseType())) {
        ...
        private void ImplementProtectedFieldAccessors(MyDictionary<string, string[]> specialNames) {
        ...
        /// </summary>
        private void OverrideMethods(Type type, MyDictionary<string, string[]> specialNames) {
            // if we have conflicting virtual's do to new slots only override the methods on the
            // most derived class.
            MyDictionary<MyKeyValuePair<string, MethodSignatureInfo>, MethodInfo> added = new MyDictionary<MyKeyValuePair<string, MethodSignatureInfo>, MethodInfo>();
            ...
            foreach (MethodInfo mi in methods) {
                MyKeyValuePair<string, MethodSignatureInfo> key = new MyKeyValuePair<string, MethodSignatureInfo>(mi.Name, new MethodSignatureInfo(mi));
            ...
            MyDictionary<PropertyInfo, PropertyBuilder> overriddenProperties = new MyDictionary<PropertyInfo, PropertyBuilder>();
        ...
        private void OverrideSpecialName(MethodInfo mi, MyDictionary<string, string[]> specialNames, MyDictionary<PropertyInfo, PropertyBuilder> overridden) {
        ...
        private bool TryOverrideProperty(MethodInfo mi, MyDictionary<string, string[]> specialNames, MyDictionary<PropertyInfo, PropertyBuilder> overridden) {
            string name;
        ...
        private void AddPublicProperty(MethodInfo mi, MyDictionary<PropertyInfo, PropertyBuilder> overridden, MethodBuilder mb, PropertyInfo foundProperty) {
        ...
        private void OverrideBaseMethod(MethodInfo mi, MyDictionary<string, string[]> specialNames) {
        ...
        private MyKeyValuePair<Type, MyDictionary<string, string[]>> SaveType(AssemblyGen ag, string name) {
            ...
            return new MyKeyValuePair<Type, MyDictionary<string, string[]>>(ret, specialNames);
        }
        ...
        private static void AddBaseMethods(Type finishedType, MyDictionary<string, string[]> specialNames) {
        ...
        private static ExtensionPropertyTracker AddPropertyInfo(Type baseType, string/*!*/ propName, MethodInfo get, MethodInfo set) {
            ...
            MyDictionary<string, List<ExtensionPropertyTracker>> propInfoList;

            if (!_overriddenProperties.TryGetValue(baseType, out propInfoList)) {
                _overriddenProperties[baseType] = propInfoList = new MyDictionary<string, List<ExtensionPropertyTracker>>();
            }
        ...
        private static void StoreOverriddenMethod(MethodInfo mi, string newName) {
            ...
            lock (PythonTypeOps._functions) {
                ...
                MyDictionary<string, List<MethodInfo>> overrideInfo;
                if (!_overriddenMethods.TryGetValue(declType, out overrideInfo)) {
                    _overriddenMethods[declType] = overrideInfo = new MyDictionary<string, List<MethodInfo>>();
                }
        ...
        private static IEnumerable<string> GetBaseName(MethodInfo mi, MyDictionary<string, string[]> specialNames) {
            string newName;
        ...
        internal static IList<MethodInfo> GetOverriddenMethods(Type type, string name) {
            MyDictionary<string, List<MethodInfo>> methods;
        ...
        internal static IList<ExtensionPropertyTracker> GetOverriddenProperties(Type type, string name) {
            lock (_overriddenProperties) {
                MyDictionary<string, List<ExtensionPropertyTracker>> props;
    ...
    sealed class ReturnFixer {
        // xiaoyi Emit opcode not supported in uwp
