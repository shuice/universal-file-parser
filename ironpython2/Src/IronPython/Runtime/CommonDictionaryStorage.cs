    ...
    internal class CommonDictionaryStorage : DictionaryStorage, ISerializable, IDeserializationCallback {
        ...
        public void AddNoLock(object key, object value) {
            if (key != null) {
                ...
                if (key.GetReleaseType() != _keyType && _keyType != HeterogeneousType) {
                    UpdateHelperFunctions(key.GetReleaseType(), key);
                }
        ...
        private void UpdateHelperFunctions(Type t, object key) {
            if (_keyType == null) {
                ...
                else if (t == typeof(Type).GetReleaseType())
                {    // this odd check checks for RuntimeType.
                    _hashFunc = _primitiveHash;
                    _eqFunc = _objectEq;
                }
                else if (t == typeof(Int64).GetInterfaces()[3])
                {
                    // .net native toolchain add a middle block, $BlockedFromReflection_1_6f81392d
                    _hashFunc = _primitiveHash;
                    _eqFunc = _objectEq;
                }
                else
       ...
       public bool Remove(object key) {
        ...
        private bool TryGetValue(Bucket[] buckets, object/*!*/ key, out object value) {
            ...
            if (_count > 0 && buckets != null) {
                int hc;
                Func<object, object, bool> eqFunc;
                string bcd = key.GetReleaseType().ToString();
                if (key.GetReleaseType() == _keyType || _keyType == HeterogeneousType) {
        ...
        public override List<MyKeyValuePair<object, object>> GetItems() {
            lock (this) {
                List<MyKeyValuePair<object, object>> res = new List<MyKeyValuePair<object, object>>(_count + (_nullValue != null ? 1 : 0));
                if (_count > 0) {
                    for (int i = 0; i < _buckets.Length; i++) {
                        Bucket curBucket = _buckets[i];
                        if (curBucket.Key != null && curBucket.Key != _removed) {
                            res.Add(new MyKeyValuePair<object, object>(curBucket.Key, curBucket.Value));
                        }
                    }
                }

                if (_nullValue != null) {
                    res.Add(new MyKeyValuePair<object, object>(null, _nullValue.Value));
                }
                return res;
            }
        }

        public override IEnumerator<MyKeyValuePair<object, object>> GetEnumerator() {
            lock (this) {
                if (_count > 0) {
                    for (int i = 0; i < _buckets.Length; i++) {
                        Bucket curBucket = _buckets[i];
                        if (curBucket.Key != null && curBucket.Key != _removed) {
                            yield return new MyKeyValuePair<object, object>(curBucket.Key, curBucket.Value);
                        }
                    }
                }

                if (_nullValue != null) {
                    yield return new MyKeyValuePair<object, object>(null, _nullValue.Value);
                }
            }
        }
        ...
        void IDeserializationCallback.OnDeserialization(object sender) {
            ...
            var buckets = (List<MyKeyValuePair<object, object>>)info.GetValue("buckets", typeof(List<MyKeyValuePair<object, object>>));

            foreach (MyKeyValuePair<object, object> kvp in buckets) {
