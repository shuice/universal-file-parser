...
namespace IronPython.Runtime {
    abstract class CustomDictionaryStorage : DictionaryStorage {
        ...
        public override List<MyKeyValuePair<object, object>> GetItems() {
            List<MyKeyValuePair<object, object>> res = _storage.GetItems();

            foreach (var item in GetExtraItems()) {
                res.Add(new MyKeyValuePair<object, object>(item.Key, item.Value));
            }
        ...
        /// </summary>
        protected abstract IEnumerable<MyKeyValuePair<string, object>> GetExtraItems();
