                ...
                switch (type.GetTypeCode()) {
                    case TypeCode.Object:
                        if (type == typeof(Complex)) {
                            ...
                            return PythonConversionBinder.ConvertToIEnumerable(conversion, Restrict(Value.GetReleaseType()));
                        } else if (type == typeof(IEnumerator)){
                            return PythonConversionBinder.ConvertToIEnumerator(conversion, Restrict(Value.GetReleaseType()));
                        } else if (type.IsSubclassOf(typeof(Delegate))) {
                            return MakeDelegateTarget(conversion, type, Restrict(Value.GetReleaseType()));
                        }
                        break;
                    case TypeCode.Int32:
