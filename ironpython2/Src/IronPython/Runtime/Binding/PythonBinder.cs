...
namespace IronPython.Runtime.Binding {
    ...
    partial class PythonBinder : DefaultBinder {
        ...
        private MyDictionary<Type/*!*/, IList<Type/*!*/>/*!*/>/*!*/ _dlrExtensionTypes;
        private bool _registeredInterfaceExtensions;    // true if someone has registered extensions for interfaces

        [MultiRuntimeAware]
        private static readonly MyDictionary<Type/*!*/, ExtensionTypeInfo/*!*/>/*!*/ _sysTypes = MakeSystemTypes();
        ...
        public void LookupMembers(CodeContext/*!*/ context, PythonType/*!*/ type, PythonDictionary/*!*/ memberNames) {
            if (!_typeMembers.IsFullyCached(type.UnderlyingSystemType, true)) {
                MyDictionary<string, MyKeyValuePair<PythonTypeSlot, MemberGroup>> members = new MyDictionary<string, MyKeyValuePair<PythonTypeSlot, MemberGroup>>();
                ...
                foreach (ResolvedMember rm in PythonTypeInfo.GetMembers(
                    ...
                    if (!members.ContainsKey(rm.Name)) {
                        members[rm.Name] = new MyKeyValuePair<PythonTypeSlot, MemberGroup>(
            ...
            foreach (MyKeyValuePair<string, PythonTypeSlot> kvp in _typeMembers.GetAllMembers(type.UnderlyingSystemType, true)) {
        ...
        /// </summary>
        public void ResolveMemberNames(CodeContext/*!*/ context, PythonType/*!*/ type, PythonType/*!*/ owner, MyDictionary<string, string>/*!*/ memberNames) {
            ...
            foreach (MyKeyValuePair<string, PythonTypeSlot> kvp in _resolvedMembers.GetAllMembers(type.UnderlyingSystemType, true)) {
                PythonTypeSlot slot = kvp.Value;
                string name = kvp.Key;
        ...
        /// </summary>
        private static MyDictionary<Type/*!*/, IList<Type/*!*/>/*!*/>/*!*/ MakeExtensionTypes() {
            MyDictionary<Type, IList<Type>> res = new MyDictionary<Type, IList<Type>>();

#if FEATURE_DBNULL
            res[typeof(DBNull)] = new Type[] { typeof(DBNullOps) };
#endif
            res[typeof(List<>)] = new Type[] { typeof(ListOfTOps<>) };
            res[typeof(MyDictionary<,>)] = new Type[] { typeof(DictionaryOfTOps<,>) };
        ...
        /// </summary>
        private static MyDictionary<Type/*!*/, ExtensionTypeInfo/*!*/>/*!*/ MakeSystemTypes() {
            MyDictionary<Type/*!*/, ExtensionTypeInfo/*!*/> res = new MyDictionary<Type, ExtensionTypeInfo>();
            ...
            res[typeof(IMyDictionary<object, object>)] = new ExtensionTypeInfo(typeof(DictionaryOps), "dict");
        ...
        private class SlotCache {
            private MyDictionary<CachedInfoKey/*!*/, SlotCacheInfo/*!*/> _cachedInfos;
            ...
            public void CacheSlot(Type/*!*/ type, bool isGetMember, string/*!*/ name, PythonTypeSlot slot, MemberGroup/*!*/ memberGroup) {
                ...
                lock (_cachedInfos) {
                    ...
                    slots.Members[name] = new MyKeyValuePair<PythonTypeSlot, MemberGroup>(slot, memberGroup);
                }
            }
            ...
            /// </summary>
            public void CacheAll(Type/*!*/ type, bool isGetMember, MyDictionary<string/*!*/, MyKeyValuePair<PythonTypeSlot/*!*/, MemberGroup/*!*/>> members) {
            ...
            /// calling this method.
            /// </summary>
            public IEnumerable<MyKeyValuePair<string/*!*/, PythonTypeSlot/*!*/>>/*!*/ GetAllMembers(Type/*!*/ type, bool isGetMember) {
                Debug.Assert(type != null);

                SlotCacheInfo info = GetSlotForType(type, isGetMember);
                Debug.Assert(info.ResolvedAll);

                foreach (MyKeyValuePair<string, PythonTypeSlot> slot in info.GetAllSlots()) {
                    if (slot.Value != null) {
                        yield return slot;
                    }
                }
            ...
            private void EnsureInfo() {
                if (_cachedInfos == null) {
                    Interlocked.CompareExchange(ref _cachedInfos, new MyDictionary<CachedInfoKey/*!*/, SlotCacheInfo>(), null);
                }
            }

            private class SlotCacheInfo {
                public SlotCacheInfo() {
                    Members = new MyDictionary<string/*!*/, MyKeyValuePair<PythonTypeSlot, MemberGroup/*!*/>>(StringComparer.Ordinal);
                }               

                public bool TryGetSlot(string/*!*/ name, out PythonTypeSlot slot) {
                    Debug.Assert(name != null);

                    MyKeyValuePair<PythonTypeSlot, MemberGroup> kvp;
                ...
                public IEnumerable<MyKeyValuePair<string/*!*/, PythonTypeSlot>>/*!*/ GetAllSlots() {
                    foreach (MyKeyValuePair<string, MyKeyValuePair<PythonTypeSlot, MemberGroup>> kvp in Members) {
                        yield return new MyKeyValuePair<string, PythonTypeSlot>(kvp.Key, kvp.Value.Key);
                    }
                }

                public MyDictionary<string/*!*/, MyKeyValuePair<PythonTypeSlot, MemberGroup/*!*/>>/*!*/ Members;
