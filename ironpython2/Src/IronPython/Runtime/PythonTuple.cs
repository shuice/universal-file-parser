...
using Utils = AnyPrefix.Microsoft.Scripting.Ast.Utils;
...
namespace IronPython.Runtime {
    ...
    public class PythonTuple : IList, IList<object>, ICodeFormattable, IExpressionSerializable, IStructuralEquatable, IStructuralComparable, IReadOnlyList<object> {
        ...
        public static PythonTuple __new__(CodeContext context, PythonType cls, object sequence) {
            ...
            if (cls == TypeCache.PythonTuple) {
                if (sequence.GetReleaseType() == typeof(PythonTuple)) return (PythonTuple)sequence;
        ...
        public virtual object __getslice__(int start, int stop) {
            ...
            if (start == 0 && stop == _data.Length &&
                this.GetReleaseType() == typeof(PythonTuple)) {
                return this;
            }
        ...
        private static PythonTuple MultiplyWorker(PythonTuple self, int count) {
            if (count <= 0) {
                return EMPTY;
            } else if (count == 1 && self.GetReleaseType() == typeof(PythonTuple)) {
                return self;
            }
