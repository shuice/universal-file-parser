...
namespace IronPython.Runtime.Operations {
    internal static class PythonTypeOps {
        private static readonly MyDictionary<FieldInfo, PythonTypeSlot> _fieldCache = new MyDictionary<FieldInfo, PythonTypeSlot>();
        private static readonly MyDictionary<BuiltinFunction, BuiltinMethodDescriptor> _methodCache = new MyDictionary<BuiltinFunction, BuiltinMethodDescriptor>();
        private static readonly MyDictionary<BuiltinFunction, ClassMethodDescriptor> _classMethodCache = new MyDictionary<BuiltinFunction, ClassMethodDescriptor>();
        internal static readonly MyDictionary<BuiltinFunctionKey, BuiltinFunction> _functions = new MyDictionary<BuiltinFunctionKey, BuiltinFunction>();
        private static readonly MyDictionary<ReflectionCache.MethodBaseCache, ConstructorFunction> _ctors = new MyDictionary<ReflectionCache.MethodBaseCache, ConstructorFunction>();
        private static readonly MyDictionary<EventTracker, ReflectedEvent> _eventCache = new MyDictionary<EventTracker, ReflectedEvent>();
        internal static readonly MyDictionary<PropertyTracker, ReflectedGetterSetter> _propertyCache = new MyDictionary<PropertyTracker, ReflectedGetterSetter>();
        ...
        internal static object CallWorker(CodeContext/*!*/ context, PythonType dt, IMyDictionary<string, object> kwArgs, object[] args) {
            object[] allArgs = ArrayOps.CopyArray(args, kwArgs.Count + args.Length);
            string[] argNames = new string[kwArgs.Count];

            int i = args.Length;
            foreach (MyKeyValuePair<string, object> kvp in kwArgs) {
                allArgs[i] = kvp.Value;
        ...
        internal static bool IsRuntimeAssembly(Assembly assembly) {
            if (assembly == typeof(PythonOps).Assembly || // IronPython.dll
                assembly == typeof(AnyPrefix.Microsoft.Scripting.Interpreter.LightCompiler).Assembly || // Microsoft.Scripting.dll
