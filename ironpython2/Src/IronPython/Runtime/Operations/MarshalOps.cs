...
namespace IronPython.Runtime.Operations {
    public class MarshalOps {
        ...
        private class MarshalWriter {
            ...
            private readonly MyDictionary<string, int> _strings;

            public MarshalWriter (int version) {
                _bytes = new List<byte> ();
                _version = version;
                if (_version > 0) {
                    _strings = new MyDictionary<string, int> ();
                }
            }
            ...
            private void WriteDict (object o) {
                ...
                _bytes.Add ((byte)'{');
                IEnumerator<MyKeyValuePair<object, object>> ie = ((IEnumerable<MyKeyValuePair<object, object>>)d).GetEnumerator ();
                while (ie.MoveNext ()) {
        ...
        private class MarshalReader {
            ...
            private readonly MyDictionary<int, string> _strings;
            private object _result;

            public MarshalReader (IEnumerator<byte> bytes) {
                _myBytes = bytes;
                _strings = new MyDictionary<int, string> ();
            }
