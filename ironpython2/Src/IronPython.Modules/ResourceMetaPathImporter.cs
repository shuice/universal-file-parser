...
namespace IronPython.Modules {
    ...
    public class ResourceMetaPathImporter {
        ...
        private readonly IMyDictionary<string, PackedResourceInfo> _unpackedLibrary;
        private readonly IMyDictionary<string, PackedResourceInfo[]> _unpackedModules;
        private readonly string _unpackingError;

        private static readonly MyDictionary<string, ModuleCodeType> SearchOrder;
        ...
        static ResourceMetaPathImporter() {
            ...
            SearchOrder = new MyDictionary<string, ModuleCodeType> {
                                                                     {
        ...
        public ResourceMetaPathImporter(Assembly fromAssembly, string resourceName) {
            ...
            _unpackedLibrary = new MyDictionary<string, PackedResourceInfo>();
            _unpackedModules = new MyDictionary<string, PackedResourceInfo[]>();
...
module, or raises ResourceImportError if it wasn't found."
        ...
        private class PackedResourceLoader {
            ...
            public bool LoadZipDirectory(
                out IMyDictionary<string, PackedResourceInfo> files,
                out IMyDictionary<string, PackedResourceInfo[]> modules,
                ...
                try {
                    ...
                    modules = moduleContents.ToMyDictionary(
                    ...
                    files = null;
                    modules = null;
                    unpackingError = String.Format("{0}: {1}", exception.GetReleaseType().Name, exception.Message);
                    return false;
                }
            }
            ...
            private bool ReadZipDirectory(out IMyDictionary<string, PackedResourceInfo> result,
                                          out string unpackingError) {
                unpackingError = null;
                result = null;
                try {
                    ...
                    using (var reader = new BinaryReader(stream)) {
                        ...
                        result = directoryEntries
                            ...
                            .ToMyDictionary(entry => entry.FullName);
                        return true;
                    }
                }
                catch (Exception exception) {
