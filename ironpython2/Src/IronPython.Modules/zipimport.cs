...
to Zip archives.";
        ...
        public class zipimporter {
            ...
            /// </summary>
            private static readonly MyDictionary<string, ModuleCodeType> _search_order;

            static zipimporter() {
                // we currently don't support bytecode, so just include the source versions.
                _search_order = new MyDictionary<string, ModuleCodeType>() {
            ...
            #region Public API
            public zipimporter(CodeContext/*!*/ context, object pathObj, [ParamDictionary] IMyDictionary<object, object> kwArgs) {
                ...
                if (!(pathObj is string)) {
                    throw PythonOps.TypeError("must be string, not {0}", pathObj.GetReleaseType());
                }
...
contain the module, but has no source for it.")]
            ...
            private byte[] GetModuleCode(CodeContext/*!*/ context, string fullname, out bool ispackage, out string modpath) {
                ...
                foreach (MyKeyValuePair<string, ModuleCodeType> entry in _search_order) {
