...
namespace IronPython.Modules {
    public class PythonDateTime {
        ...
        public class date : ICodeFormattable {
            ...
            // instance methods
            public virtual date replace(CodeContext/*!*/ context, [ParamDictionary]IMyDictionary<object, object> dict) {
                int year2 = _dateTime.Year;
                int month2 = _dateTime.Month;
                int day2 = _dateTime.Day;

                foreach (MyKeyValuePair<object, object> kvp in (IMyDictionary<object, object>)dict) {
            ...
            internal bool CheckType(object other, bool shouldThrow) {
                ...
                if (other.GetReleaseType() != GetType()) {
                    // if timetuple is defined on the other object go ahead and let it try the compare,
                    // but only if it's a user-defined object
                    if (!(GetType() == typeof(date) && other.GetReleaseType() == typeof(datetime) ||
                        GetType() == typeof(datetime) & other.GetReleaseType() == typeof(date))) {
            ...
            public static object operator >(date self, object other) {
                ...
                return AnyPrefix.Microsoft.Scripting.Runtime.ScriptingRuntimeHelpers.BooleanToObject(self.CompareTo(other) > 0);
            }
        ...
        public class datetime : date, ICodeFormattable {
            ...
            public override date replace(CodeContext/*!*/ context, [ParamDictionary]IMyDictionary<object, object> dict) {
                int lyear = year;
                int lmonth = month;
                int lday = day;
                int lhour = hour;
        ...
        public class time : ICodeFormattable {
            ...
            public object replace([ParamDictionary]IMyDictionary<object, object> dict) {
                int lhour = hour;
                int lminute = minute;
                int lsecond = second;
                int lmicrosecond = microsecond;
