...
{
    public sealed partial class ElementEditCustom : ElementEditUserControl
    {
        public ElementEditCustom()
        {
        ...
        protected override void LoadDataToUI()
        {
            ...
            string scriptId = this.elementBase.GetValue(ElementKey.script);
            EmbedScript embedScript = this.elementBase.grammar.GetScript(Grammar.script_type_datatype).FirstOrDefault(item => $"id:{item.id}" == scriptId);
            if (embedScript != null)
            {
                TextBlock_Script_Name.Text = embedScript.name;
            }            
